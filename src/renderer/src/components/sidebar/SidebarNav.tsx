...
const SidebarNav = React.memo(function SidebarNav() {
  ...
  const flowsActive = activeView === 'flows'
  ...
  return (
    ...
    >
      ...
      <button
        type="button"
        onClick={openFlowsPage}
        aria-current={flowsActive ? 'page' : undefined}
        className={cn(
          'flex w-full items-center gap-2 rounded-md px-2 py-1.5 text-left text-[13px] font-medium tracking-tight transition-colors',
          flowsActive
            ? 'bg-worktree-sidebar-accent text-worktree-sidebar-accent-foreground'
      ...
      >
        <Workflow
          className={cn('size-4 shrink-0', !flowsActive && 'text-worktree-sidebar-foreground/30')}
          strokeWidth={flowsActive ? 2.25 : 1.75}
        />
        <span className="flex-1">
          {translate('auto.components.sidebar.SidebarNav.flows', 'Flows')}
        </span>
      </button>
